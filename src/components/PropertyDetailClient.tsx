...
export default function PropertyDetailClient({ property }: PropertyDetailClientProps) {
  ...
  // Verificar si el usuario puede volver (navegó desde nuestra página principal)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#F2EFE9]">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 sm:py-6 md:py-8">
        ...
        {/* Info text about rental type - MEJORADO: Más compacto y legible */}
        <div className="mb-4 sm:mb-6 bg-white border-l-4 border-[#2C2621] p-3 sm:p-4">
          <p className="font-mono text-[10px] sm:text-xs text-[#2C2621]/80 leading-relaxed">
            <strong className="text-[#2C2621]">{isDaily ? t.dailyRental : t.monthlyRental}</strong>
            {' · '}
            <span className="hidden sm:inline">{isDaily ? t.dailyInfo : t.monthlyInfo}</span>
            <span className="sm:hidden">
              {isDaily 
                ? 'Estancias breves ideales para turismo o necesidades puntuales.' 
                : 'Estancias largas (1-11 meses) para estudios o trabajo.'}
            </span>
          </p>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="lg:sticky lg:top-8 bg-white border-2 border-[#2C2621]/20 p-4 sm:p-6 rounded-lg shadow-lg">
              ...
              <button 
                ...
                className="w-full rounded-none border-2 border-[#3B332B] px-4 sm:px-6 py-2 sm:py-3 font-sans font-bold uppercase tracking-widest text-xs sm:text-sm transition-all duration-300 bg-[#3B332B] text-white hover:bg-transparent hover:text-[#3B332B]"
              >
                {t.reserveNow}
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            ...
            {property.description && (
              <div>
                {/* Desktop: Title + Read More button */}
                <div className="hidden md:block">
                  <h2 className="font-sans text-xl sm:text-2xl font-bold text-[#2C2621] uppercase tracking-tight mb-4 sm:mb-6 border-b-2 border-[#2C2621]/20 pb-3 sm:pb-4">
                    {t.aboutPlace}
                  </h2>
                  <div className="bg-white p-4 sm:p-6 rounded-lg border border-[#2C2621]/10">
                    <p className={`font-mono text-xs sm:text-sm text-[#2C2621] leading-loose whitespace-pre-line ${
                    ...
                    <button
                      ...
                      className="mt-3 font-mono text-xs text-[#2C2621]/70 hover:text-[#2C2621] underline transition-colors duration-300"
                    >
                      {isDescriptionExpanded ? (language === 'es' ? 'Leer menos' : 'Read less') : (language === 'es' ? 'Leer más' : 'Read more')}
                    </button>
                  </div>
                </div>
                
                {/* Mobile: Accordion with clickable title */}
                <div className="md:hidden">
                  <button
                    onClick={() => setIsDescriptionExpanded(!isDescriptionExpanded)}
                    className="w-full flex items-center justify-between text-left font-sans text-xl sm:text-2xl font-bold text-[#2C2621] uppercase tracking-tight mb-4 sm:mb-6 border-b-2 border-[#2C2621]/20 pb-3 sm:pb-4 hover:bg-[#2C2621]/5 transition-colors duration-300 rounded-lg px-2 py-1 -mx-2"
                  >
                    <span>{t.aboutPlace}</span>
                    <svg 
                      xmlns="http://www.w3.org/2000/svg" 
                      width="24" 
                      height="24" 
